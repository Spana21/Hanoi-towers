from ClassDiscIni import *
from ClassSloupce import *

class Disky():
    def __init__(self, number):
        """
               This constructor creates the specified number of Disc objects and stores them in the `s1.discs` list.
               The number of Disc objects is determined by the `number` parameter.

               :param number: number of Disc objects to create and store in `s1.discs` list
               :type number: int
               """
        if number == 2:
